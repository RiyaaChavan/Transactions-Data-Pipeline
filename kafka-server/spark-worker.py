# kafka-server/spark-worker.py
print("Started Spark Worker")
import os
...
spark = (
    ...
    .master("spark://spark-master:7077")  # Use Spark master service in Docker Compose
    .getOrCreate()
)
print(1)
...
try:
    ...
    df = (
        ...
        .option("kafka.bootstrap.servers", "kafka:29092")  # Use Kafka service in Docker Compose
    ...
    def write_to_timescaledb(df, epoch_id):
        try:
            ...
            df.write.format("jdbc").option("driver", "org.postgresql.Driver").option(
                "url", "jdbc:postgresql://timescaledb:5432/financial_transactions"
